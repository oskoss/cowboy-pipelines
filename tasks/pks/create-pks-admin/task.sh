...
echo "- Your PKS API endpoint [$PKS_UAA_FQDN] should be routable and accessible from the Concourse worker(s) network."
...
echo "Connecting to PKS UAA server [$PKS_UAA_FQDN]..."
# login to PKS UAA
uaac target https://$PKS_UAA_FQDN:8443 --skip-ssl-validation
...
if [ -z "$CHK" ]; then
  echo "Creating new PKS CLI administrator user per PKS tile documentation https://docs.pivotal.io/runtimes/pks/1-0/manage-users.html#uaa-scopes"
...
echo "Example: "
echo "   pks login -a $PKS_UAA_FQDN -u $PKS_CLI_USERNAME -p <pks-cli-password-provided>"