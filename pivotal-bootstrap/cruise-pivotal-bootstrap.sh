...
/usr/bin/concourse quickstart \
  ...
  --postgres-user="vcap" \
  --tsa-host-key="/data/pipelines/concourse-keys/tsa_host_key" \
  --tsa-authorized-keys="/data/pipelines/concourse-keys/authorized_worker_keys" \
  --session-signing-key="/data/pipelines/concourse-keys/session_signing_key" \
  --worker-tsa-worker-private-key="/data/pipelines/concourse-keys/worker_key" \
  --worker-tsa-public-key="/data/pipelines/concourse-keys/tsa_host_key.pub" \
...
cat << EOF > /data/pipelines/cowboy-pipelines/pipelines/cruise-edge-deploy-params.yaml
...
cat /data/pipelines/cowboy-pipelines/pipelines/cruise-edge-deploy-params.yaml >>$STATE
...
if [ "$RESET_PIPELINE" = "True" ]; then
    ...
    fly -t local set-pipeline -n -p initial-deploy -c /data/pipelines/cowboy-pipelines/pipelines/cruise-edge-deploy-pipeline.yml --load-vars-from=/data/pipelines/cowboy-pipelines/pipelines/cruise-edge-deploy-params.yaml >>$STATE 2>&1
